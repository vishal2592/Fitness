...
import {
    Plus, Search, MoreVertical, Pencil, Trash2, Eye, X,
    CalendarDays, Clock3, UserRound, Users, Video, MapPin
} from "lucide-react";

const Classes = () => {
    const categories = [
        "Strength Training", "HIIT & Cardio", "Yoga & Flexibility",
        "Zumba", "Pilates", "Weight Loss",
    ];
    ...
    const [classes, setClasses] = useState([
        {
            id: 1, name: "Morning HIIT Blast", category: "HIIT & Cardio",
            trainer: "Sarah Miller", date: "2026-08-08", time: "07:00 AM",
            duration: 45, capacity: 25, enrolled: 18, type: "Live",
            status: "Scheduled", location: "Online",
            description: "High intensity workout focused on stamina and cardio.",
        },
        {
            id: 2, name: "Full Body Strength", category: "Strength Training",
            trainer: "Alex Johnson", date: "2026-08-09", time: "06:30 PM",
            duration: 60, capacity: 20, enrolled: 15, type: "Live",
            status: "Scheduled", location: "Online",
            description: "Full body strength workout for intermediate members.",
        },
        {
            id: 3, name: "Beginner Yoga Flow", category: "Yoga & Flexibility",
            trainer: "Emma Davis", date: "2026-08-10", time: "08:00 AM",
            duration: 45, capacity: 30, enrolled: 22, type: "Live",
            status: "Scheduled", location: "Online",
            description: "Gentle yoga session focused on flexibility and mobility.",
        },
    ]);

    const emptyForm = {
        name: "", category: "", trainer: "", date: "", time: "",
        duration: "", capacity: "", type: "Live", location: "Online",
        description: "",
    };
    ...
    const stats = [
        ["Total Classes", classes.length, CalendarDays, "blue"],
        ["Scheduled", classes.filter(c => c.status === "Scheduled").length, CalendarDays, "green"],
        ["Live Classes", classes.filter(c => c.type === "Live").length, Video, "purple"],
        ["Total Enrolled", classes.reduce((a, c) => a + c.enrolled, 0), Users, "orange"],
    ];
    ...
    return (
        <div className="min-h-screen space-y-5 p-4 md:p-6">
            ...
            <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
                ...
                <button onClick={() => { setSelected(null); setForm(emptyForm); setModal("form"); }}
                    className="flex items-center justify-center gap-2 rounded-xl bg-blue-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-blue-600">
            ...
            <div className="flex flex-col gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm md:flex-row dark:border-darkTheme-border dark:bg-darkTheme-card">
                <div className="relative flex-1">
                    ...
                    <input value={search} onChange={e => setSearch(e.target.value)}
                        ...
                        className="w-full rounded-xl border border-gray-200 bg-gray-50 py-2.5 pl-10 pr-4 text-sm outline-none dark:border-darkTheme-border dark:bg-darkTheme-border/30 dark:text-darkTheme-text" />
                </div>

                <select value={category} onChange={e => setCategory(e.target.value)}
                    className="rounded-xl border border-gray-200 bg-gray-50 px-4 py-2.5 text-sm dark:border-darkTheme-border dark:bg-darkTheme-border/30 dark:text-darkTheme-text">
                    <option>All</option>
                    ...
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 dark:bg-darkTheme-border/30">
                            <tr className="text-left text-gray-500 dark:text-darkTheme-muted">
                                {["Class", "Trainer", "Date & Time", "Duration", "Members", "Type", "Status", "Action"].map(h =>
                        ...
                        <tbody>
                            {filtered.map(c => (
                                <tr key={c.id} className="border-b border-gray-100 last:border-0 dark:border-darkTheme-border">
                                    ...
                                    <td className="px-4 py-4">{c.trainer}</td>
                                    <td className="px-4 py-4">{c.date}<br /><span className="text-xs text-gray-500">{c.time}</span></td>
                                    <td className="px-4 py-4">{c.duration} min</td>
                                    <td className="px-4 py-4">{c.enrolled}/{c.capacity}</td>
                                    <td className="px-4 py-4"><span className="rounded-full bg-blue-100 px-2.5 py-1 text-xs text-blue-700">{c.type}</span></td>
                                    <td className="px-4 py-4"><span className="rounded-full bg-green-100 px-2.5 py-1 text-xs text-green-700">{c.status}</span></td>
                    ...
                    <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white dark:bg-darkTheme-card">
                        ...
                        <form onSubmit={submit} className="grid gap-4 p-5 md:grid-cols-2">
                            {[
                                ...
                                <div key={name}>
                                    ...
                                    <input name={name} type={type} value={form[name]} onChange={e => setForm({ ...form, [name]: e.target.value })}
                                        className="w-full rounded-xl border px-3 py-2.5 text-sm dark:border-darkTheme-border dark:bg-darkTheme-border/30 dark:text-darkTheme-text" />
                                </div>
                            ))}

                            <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })}
                                className="rounded-xl border px-3 py-2.5 text-sm dark:border-darkTheme-border dark:bg-darkTheme-border/30 dark:text-darkTheme-text">
                            ...
                            <textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })}
                                ...
                                className="rounded-xl border p-3 text-sm md:col-span-2 dark:border-darkTheme-border dark:bg-darkTheme-border/30 dark:text-darkTheme-text" rows="3" />
